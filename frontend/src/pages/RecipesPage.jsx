...
const CAT_STYLE = {
  soap:    { bg: "#4A7FA822", text: "#306080" },
  lotion:  { bg: "#6D9B5822", text: "#4E7A3C" },
  lip_balm:{ bg: "#C97B5A22", text: "#8F4A2C" },
  candle:  { bg: "#8B691422", text: "#4A380C" },
  other:   { bg: "#8B691422", text: "#4A380C" },
};
  ...
  return (
    <Shell>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#8B6914", cursor: "pointer", marginLeft: "auto" }}>
...
function RecipeCard({ recipe, cpu, onEdit, onScale, onExport, onBatch }) {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.88)", borderRadius: 18, border: "1px solid #E8D5B4", overflow: "hidden", transition: "all 0.2s", boxShadow: hovered ? "0 10px 32px rgba(46,34,8,0.14)" : "0 2px 8px rgba(46,34,8,0.06)", transform: hovered ? "translateY(-2px)" : "none" }}
      ...
      <div style={{ padding: "18px 20px 16px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono'", color: "#8B6914", background: "#F3EAD6", padding: "2px 7px", borderRadius: 4 }}>v{recipe.version}</span>
        </div>

        <h3 style={{ fontSize: 17, fontFamily: "'Playfair Display'", lineHeight: 1.3, marginBottom: 8 }}>{recipe.name}</h3>

        {recipe.description && (
          <p style={{ fontSize: 13, color: "#8B6914", lineHeight: 1.5, marginBottom: 14, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
            {recipe.description}
          </p>
        )}

        {/* Stats row */}
        <div style={{ display: "flex", gap: 12, fontSize: 12, color: "#6B5010", marginBottom: 16, flexWrap: "wrap" }}>
          ...
          {cpu != null && <Stat icon="💰" label={`$${cpu.toFixed(2)}/unit`} color="#4E7A3C" />}
          {cpu != null && <Stat icon="🏷️" label={`$${(cpu * 3).toFixed(2)} retail`} color="#8B6914" />}
        </div>
        ...
        {recipe.ingredients?.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 16 }}>
            {recipe.ingredients.slice(0, 5).map(ri => (
              <span key={ri.id} style={{ fontSize: 11, padding: "2px 7px", background: "#F3EAD6", borderRadius: 4, color: "#4A380C" }}>{ri.ingredient_name}</span>
            ))}
            {recipe.ingredients.length > 5 && (
              <span style={{ fontSize: 11, padding: "2px 7px", background: "#E8D5B4", borderRadius: 4, color: "#6B5010" }}>+{recipe.ingredients.length - 5} more</span>
            )}
          </div>
        )}

        {/* Actions */}
        <div style={{ display: "flex", gap: 6, borderTop: "1px solid #F3EAD6", paddingTop: 14 }}>
...
function Stat({ icon, label, color }) {
  return (
    <span style={{ display: "flex", alignItems: "center", gap: 3, color: color || "#6B5010" }}>
      {icon} {label}
    </span>
  );
}
...
function ScaleModal({ recipe, ingredients, onClose, onBatch }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(46,34,8,0.4)", backdropFilter: "blur(4px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#FAF6ED", borderRadius: 20, border: "1px solid #E8D5B4", width: "100%", maxWidth: 640, maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(46,34,8,0.2)" }}>
        ...
        <div style={{ padding: 24 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {[["yield", "By Yield"], ["units", "By Units"], ["factor", "By Factor"]].map(([m, l]) => (
              <button key={m} onClick={() => { setMode(m); setResult(null); }} style={{ flex: 1, padding: "8px", borderRadius: 8, border: "1px solid", cursor: "pointer", fontSize: 13, fontWeight: 500, borderColor: mode === m ? "#6B5010" : "#E8D5B4", background: mode === m ? "#6B5010" : "transparent", color: mode === m ? "#fff" : "#6B5010", fontFamily: "inherit" }}>{l}</button>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "flex-end" }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#6B5010", marginBottom: 4 }}>
          ...
          <div style={{ padding: "10px 14px", background: "#F3EAD6", borderRadius: 10, marginBottom: 16, fontSize: 13, color: "#4A380C" }}>
          ...
          {result && (
            <div>
              {result.ingredients.map(ri => {
                ...
                return (
                  <div key={ri.ingredient_id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: hasEnough ? "#FDFBF7" : "#FFF0EB", borderRadius: 8, marginBottom: 4, border: `1px solid ${hasEnough ? "#F3EAD6" : "#C97B5A40"}` }}>
                    <div>
                    ...
                    <div style={{ display: "flex", gap: 16, fontSize: 13, alignItems: "center" }}>
                      {ri.line_cost && <span style={{ color: "#6D9B58" }}>${ri.line_cost.toFixed(2)}</span>}
                      <span style={{ fontFamily: "'JetBrains Mono'", fontWeight: 600, color: hasEnough ? "#4A380C" : "#B5603C" }}>{Number(ri.amount_scaled).toFixed(2)} {ri.unit}</span>
                    </div>
                  </div>
                );
              })}
              <div style={{ marginTop: 16, padding: "14px 16px", background: "#2E2208", borderRadius: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ color: "rgba(250,246,237,0.7)", fontSize: 14 }}>
                  Total: <strong style={{ color: "#FAF6ED" }}>${result.total_cost?.toFixed(2)}</strong>
                  {result.cost_per_unit && <span> · <strong style={{ color: "#8FAF7E" }}>${result.cost_per_unit.toFixed(2)}/unit</strong></span>}
                </span>
...
function EmptyState({ onAdd }) {
  return (
    <div style={{ textAlign: "center", padding: "80px 40px", background: "rgba(255,255,255,0.6)", borderRadius: 20, border: "2px dashed #E8D5B4" }}>
      <div style={{ fontSize: 48, marginBottom: 16 }}>📋</div>
      <h3 style={{ fontSize: 22, fontFamily: "'Playfair Display'", marginBottom: 8 }}>No recipes yet</h3>
      <p style={{ color: "#8B6914", marginBottom: 24, fontSize: 14 }}>Create your first formula to get started</p>
...
function Chip({ label, active, onClick }) {
  return <button onClick={onClick} style={{ padding: "5px 12px", borderRadius: 16, border: "1px solid", fontSize: 12, fontWeight: 500, cursor: "pointer", transition: "all 0.15s", borderColor: active ? "#6B5010" : "#E8D5B4", background: active ? "#6B5010" : "transparent", color: active ? "#FDFBF7" : "#6B5010", fontFamily: "inherit" }}>{label}</button>;
}
